import X2JS from "jsonconverter";
import { $ } from "jquery";
...
function parserToTable(jsonObjects) {
  for (var i = 0; i < jsonObjects.length; i++) {
    if (
      insertedKeys.indexOf(jsonObjects[i].Comprobante._sello) == -1 ||
      insertedKeys.indexOf(jsonObjects[i].Comprobante._Sello) == -1
    ) {
      //console.log("nuevo row", insertedKeys.indexOf(jsonObjects[i].Comprobante));

      var row = parserRowAsInvoiceMX(
        jsonObjects[i].Comprobante,
        fileNames[i],
        i + 1
      );
      insertRow(row);
...
function createTableDivision(...args) {
  return args.reduce(
    (accumulator, currentValue) => accumulator + `<td>${currentValue}</td>`,
    ""
  );
}
  ...
  if (concepto.length > 1) {
    for (i = 0; i < concepto.length; i++) {
      if (i == 0) {
        ...
        if (version <= 3.2) {
          conceptosAgrupados =
            conceptosAgrupados + "<br>" + concepto[i]._descripcion;
        }
        if (version == 3.3) {
          conceptosAgrupados =
            conceptosAgrupados + "<br>" + concepto[i]._Descripcion;
